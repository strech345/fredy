...
/**
 * Remove leftover puppeteer temp directories from previous crashed sessions.
 * Should be called once at application startup.
 */
export function cleanupStalePuppeteerDirs() {
  try {
    const tmpDir = os.tmpdir();
    const entries = fs.readdirSync(tmpDir);
    let cleaned = 0;
    for (const entry of entries) {
      if (entry.startsWith('puppeteer-fredy-')) {
        try {
    ...
    if (cleaned > 0) {
      logger.info(`[puppeteer] Cleaned up ${cleaned} stale temp dir(s) from previous sessions.`);
    }
  } catch {
    // ignore
  }
...
export async function closeBrowser(browser) {
  ...
  // Capture the browser process PID before closing, so we can force-kill it
  // if browser.close() fails (e.g. Chromium already crashed)
  let browserPid;
  try {
  ...
  try {
    ...
    // browser.close() failed (e.g. Chromium already crashed).
    // Force-kill the Chromium process tree so it doesn't become a zombie.
    if (browserPid) {
      try {
        // SIGKILL the entire process group to also kill renderer/crashpad sub-processes
